                ...
                <DragDropContext onDragEnd={(result) => {
                  ...
                  <Droppable droppableId="mappings-list">
                    {(provided, snapshot) => (
                      ...
                      {/* Header */}
                      <div className="grid gap-3 px-4 py-2 text-xs font-semibold text-slate-600 bg-slate-100 border-b border-slate-200 sticky top-0 z-10" style={{gridTemplateColumns:"32px 180px 80px 200px 140px 120px 80px"}}>
                        <span></span><span>Source</span><span>Type</span><span>Target</span><span>Transform</span><span>DQ Rule</span><span></span>
                      </div>
                      {filteredMappings.length === 0 && (
                        <div className="text-center py-6 text-xs text-slate-400">No mappings match "{mappingSearch}"</div>
                      )}
